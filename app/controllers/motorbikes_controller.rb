class MotorbikesController < ApplicationController
  ...
  def create
    ...
    if @motorbike.save
      redirect_to listing_motorbike_path(@motorbike), notice: "Saved..."
    else
  ...
  def is_conflict(start_date, end_date, motorbike)
    ...
    def is_authorised
      redirect_to root_path, alert: "You don't have permission" unless current_user.id == @motorbike.user_id
    end
